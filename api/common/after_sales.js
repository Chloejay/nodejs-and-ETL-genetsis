  ...
  Model.once('attached', (a) => {
    ...
    Model.observe('after save', (ctx, next) => {
      ...
      if (ctx.instance) {
        ...
        Object.keys(Model.relations).forEach((related) => {
          let promiseArray = [];
          if (ctx.hookState.relations[related] != undefined) {
            ...
            if (typeof data !== 'undefined') {
              data.forEach((value, key) => {
                ...
                value.id = ids[key];
                value.afterSalesId = ctx.instance.aftersale_no; // eslint-disable-line
                ctx.instance[related].exists(value.id, (err, exists) => {
                  if (!exists) {
                    promise = ctx.instance[related].create(value);
                  } else {
                    promise = ctx.instance[related].updateById(value.id, value); // eslint-disable-line
                  }
                });
